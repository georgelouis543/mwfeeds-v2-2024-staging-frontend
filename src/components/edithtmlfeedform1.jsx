...
const EditHtmlFeedform1 = (props) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const response = await axiosPrivate.get(`/html_feed_handler/get_feed_params?feed_id=${props.feed_id}`); 
    ...
    fetchInitialData();
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        try {
          const response = await axiosPrivate.post('html_feed_handler/feed_mapper', formData, {
            headers: {
...
export default EditHtmlFeedform1
