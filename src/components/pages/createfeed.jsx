...
const Createfeed = () => {
  ...
  const handleCreateFeed = async () => {
    try {
      const response = await axiosPrivate.post('/feed_handler/save_feed', allformdata, {
        headers: {
  ...
  return (
    <div className='flex justify-between items-top w-full h-full px-2 2xl:px-16 py-7'>
      <div className='w-[35%] flex flex-col'>
        ...
        <div>
        <Createfeedform onChangeCreateFeedform = {onChangeCreateFeedformData}/>
        </div>
        <div>
        ...
        {HtmlRawData.length > 0 && 
              <SetitemForm items_arr={HtmlRawData} isJavascriptEnabled={isJavascriptEnabled} isRssfeed={isRssFeed} onChangesetItemForm = {onChangeSetItemFormData}/>
        }
        </div>
      </div>
